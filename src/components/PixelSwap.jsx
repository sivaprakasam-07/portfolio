...
// Keep max pixels low (capped at 32) so DOM node creation, cloning,
// and Web Animation instances stay extremely lightweight.
const MAX_PIXELS = 32;
...
const buildKeyframes = ({ ease, startScale, endScale, spin, fade }) => {
  ...
  for (let step = 0; step <= KEYFRAME_STEPS; step += 1) {
    ...
    windowKeyframes.push({
      ...
      transform: `translate3d(0, 0, 0) ${angle ? `rotate(${angle}deg)` : ''} scale(${scale})`
    });
    contentKeyframes.push({
      offset: progress,
      transform: `scale(${1 / scale}) ${angle ? `rotate(${-angle}deg)` : ''} translate3d(0, 0, 0)`
    });
  }
...
function PixelSwap({
  ...
  // Handle rapid pointer interactions and queue interruption
  useEffect(() => {
    if (transition) {
      // If user rapidly reversed direction (e.g. pointer quickly left), cancel ongoing animation immediately
  ...
  useEffect(() => {
    ...
    const total = Math.max(150, settings.duration);
    ...
    // Clone source layer once as a lightweight template
  ...
  const interactionProps = useMemo(() => {
    if (trigger === 'hover') {
      if (isTouchDevice) {
        // On touch devices, do not attach hover listeners to prevent scrolling lag; toggle on tap
        return {
    ...
    return {};
  }, [desiredActive, isTouchDevice, requestActive, trigger]);
